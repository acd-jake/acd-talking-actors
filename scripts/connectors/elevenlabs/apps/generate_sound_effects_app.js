...
export class GenerateSoundEffectsApp extends Application {
    ...
    async generateEffectInternal() {
        const savePath = game.settings.get(this.connector.mainSettingsId, ELEVENLABS_CONSTANTS.SOUNDEFFECTFOLDER);
        ...
        const playListName = game.settings.get(this.connector.mainSettingsId, ELEVENLABS_CONSTANTS.SOUNDEFFECTPLAYLIST);
    ...
    async generateSoundEffect(text, settings = {}) {
        // Show busy overlay
        this.showOverlay();
        let response = await new api.SoundGenerationRequest(this.connector, text, settings).fetch();
        // Hide busy overlay
        this.hideOverlay();
        if (!response.ok) {
